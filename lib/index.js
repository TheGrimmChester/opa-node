...
function start(options) {
  ...
  try {
    ...
    // Error triage: uncaughtException / unhandledRejection + captureException API.
    state.errors = new Errors(state);
    state.errors.installHooks();
    // Profiling: optional CPU sampling profiler (OPA_PROFILING=1).
    if (config.profiling) {
    ...
    // Agent coverage: fleet heartbeat / remote config (kill-switch, sampling).
...
module.exports = {
  ...
  wrapLambdaHandler: (handler) => require('./serverless').wrapHandler(state, handler),
  extractEventTraceparent: require('./serverless').extractTraceparent,
  iast: {
    checkSQL: (sql, meta) => require('./iast').checkSQL(state, sql, meta),
    checkCommand: (cmd, meta) => require('./iast').checkCommand(state, cmd, meta),
    checkFile: (path, meta) => require('./iast').checkFile(state, path, meta),
    installHooks: () => require('./iast').installHooks(state)
  },
