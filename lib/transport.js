...
class Transport {
  ...
  /** Serverless: drain queued lines for HTTP ingest (does not require TCP). */
  drainLines() {
  ...
  /**
   * Serverless: flush with a hard deadline — for Lambda freeze, not process exit.
   * Bounded time (not disk). Resolves when flushed or deadline hits.
   */
  flushSync(timeoutMs) {
    const limit = typeof timeoutMs === 'number' ? timeoutMs : 400;
    return new Promise((resolve) => {
      try {
        ...
        if (this.queue.length === 0) return resolve(true);
        this._ensureConnection();
        const deadline = setTimeout(() => resolve(false), limit);
